...
from agents.unreal import Saver, Agent, Trainer
...
class Runner(BaseRunner):
    def __init__(self, env_args, log_path, model_path):
    ...
    def run(self):
        ...
        params = [{'params': self.global_network.base_conv_net.parameters()},
                  {'params': self.global_network.base_lstm_net.parameters()},
                  {'params': self.global_network.base_value_net.parameters()},
                  {'params': self.global_network.base_net.parameters()}]
        if FLAGS.use_pixel_change:
            params.append({'params': self.global_network.pc_net.parameters()})
        if FLAGS.use_reward_prediction:
            params.append({'params': self.global_network.rp_net.parameters()})
        optimizor = SharedAdam(params=params, lr=initial_learning_rate,
