...
class ObsProcesser:
    ...
    def process_one_input(self, timestep: TimeStep):
        ...
        pp_obs = {
            FEATURE_KEYS.screen_numeric: self.get_screen_numeric(obs).astype(np.float32),
            FEATURE_KEYS.screen_unit_type: obs["screen"][SCREEN_FEATURES.unit_type.index].astype(np.longlong),
            FEATURE_KEYS.minimap_numeric: self.get_mimimap_numeric(obs).astype(np.float32),
            FEATURE_KEYS.available_action_ids: get_available_actions_flags(obs).astype(np.float32),
            FEATURE_KEYS.player_relative_screen: obs["screen"][
                SCREEN_FEATURES.player_relative.index].astype(np.longlong),
            FEATURE_KEYS.player_relative_minimap: obs["minimap"][
                MINIMAP_FEATURES.player_relative.index].astype(np.longlong)
        }
...
class ActionProcesser:
    ...
    def combine_batch(self, mb_actions):
        ...
        d[FEATURE_KEYS.selected_spatial_action] = np.stack(k[1] for k in mb_actions).astype(np.float32)
        d[FEATURE_KEYS.is_spatial_action_available] = self.is_spatial[
            d[FEATURE_KEYS.selected_action_id]
        ].astype(np.float32)  # bool to float
        d[FEATURE_KEYS.selected_action_id] = d[FEATURE_KEYS.selected_action_id].astype(np.float32)
        return d
